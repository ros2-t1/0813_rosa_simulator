...
class DeliveryConfirmationGUI(Node):
    ...
    def show_confirmation_dialog(self):
        ...
        question_label = tk.Label(self.confirmation_window, text=question_text, 
...
배달시간: {time.strftime('%H:%M:%S', time.localtime(float(self.current_task_info['delivery_time'])))}
        ...
        # YES 버튼 (초록색)
        yes_button = tk.Button(button_frame, text="예 (Y)", font=("Arial", 12, "bold"),
                              bg="lightgreen", fg="darkgreen", width=8, height=2,
                              command=lambda: self.send_response("YES"))
        yes_button.pack(side=tk.LEFT, padx=10)
        
        # NO 버튼 (빨간색)
        no_button = tk.Button(button_frame, text="아니오 (N)", font=("Arial", 12, "bold"),
    ...
    def send_response(self, response):
        ...
        try:
            ...
            if response == "YES":
                ...
                messagebox.showwarning("대기", f"{self.current_task_info['item']} 수령이 거부되었습니다.\n로봇이 현재 위치에서 대기합니다.")
