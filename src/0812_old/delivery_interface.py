...
class DeliveryInterface(Node):
    ...
    def __init__(self, task_manager_callback=None):
        super().__init__('delivery_interface')
        
        # TaskManager와의 연결을 위한 콜백 함수
        self.task_manager_callback = task_manager_callback
        
        # GUI 수령확인 관련 토픽
        self.delivery_confirm_pub = self.create_publisher(String, '/rosa/delivery_confirmation_request', 10)
        self.create_subscription(String, '/rosa/delivery_confirmation_response', self.delivery_confirmation_callback, 10)
        
        # DB 로그 관련 토픽 (필요시 추가)
        self.db_log_pub = self.create_publisher(String, '/rosa/db_log', 10)
        
        self.get_logger().info("✅ DeliveryInterface 초기화 완료 (GUI/DB 연동 전담)")
    ...
    def request_delivery_confirmation(self, task_info):
        ...
        try:
            ...
            self.get_logger().info(f"📋 GUI 확인 요청 발송: {task_info['user_id']}님께 '{task_info['item']}' 수령확인 요청")
            self.get_logger().info(f"📋 업무 ID: {task_info['order_id']}, 배달지: {task_info['destination']}")
            
        except Exception as e:
            self.get_logger().error(f"❌ GUI 확인 요청 발송 실패: {e}")
    ...
    def delivery_confirmation_callback(self, msg: String):
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                self.get_logger().info(f"📨 GUI 응답 수신: {user_id}님이 업무 {order_id}에 대해 '{response}' 응답")
                ...
                self.get_logger().warn(f"⚠️ 잘못된 GUI 응답 형식: {msg.data}")
                
        except Exception as e:
            self.get_logger().error(f"❌ GUI 응답 처리 중 오류: {e}")
    ...
    def log_to_database(self, task_info, status):
        ...
        try:
            ...
            self.get_logger().info(f"📊 DB 기록: {task_info['user_id']}님의 {task_info['item']} → {task_info['destination']} ({status}, 총 {total_time:.0f}초)")
            
        except Exception as e:
            self.get_logger().error(f"❌ DB 로그 기록 실패: {e}")
...
def main(args=None):
    ...
    try:
        rclpy.spin(delivery_interface)
    except KeyboardInterrupt:
        pass
    finally:
        delivery_interface.destroy_node()
        rclpy.shutdown()
