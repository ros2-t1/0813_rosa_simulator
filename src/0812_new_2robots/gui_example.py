...
class DeliveryConfirmationGUI(Node):
    ...
    def show_confirmation_dialog(self):
        ...
        # 창이 완전히 표시된 후에 grab_set 호출 (에러 방지)
        self.confirmation_window.after(100, self.set_window_grab)
...
배달시간: {time.strftime('%H:%M:%S', time.localtime(float(self.current_task_info['delivery_time'])))}
    ...
    def set_window_grab(self):
        """창이 완전히 표시된 후 grab_set 호출 (안전한 방식)"""
        try:
            ...
            self.get_logger().warn(f"⚠️ Window grab 설정 실패 (무시 가능): {e}")
